...
// ✅ Function: Fetch News from Gemini
function getNewsFromGemini($api_url, $category) {
    $prompt = [
        "contents" => [[
            "parts" => [[
                "text" => "You are a professional $category news writer.\n\nGenerate 3 unique and fresh $category news articles. 
Each article should have a catchy title (first line) followed by 3–5 paragraphs of content. 
Focus on current $category trends in Kenya. Make sure they are different from previous ones. 
...
foreach ($categories as $category) {
    $gemini_response = getNewsFromGemini($api_url, $category);

    if ($gemini_response) {
        $articles = preg_split("/\n\s*\n/", trim($gemini_response));
        $inserted = 0;
        $skipped = 0;
